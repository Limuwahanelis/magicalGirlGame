...
public class AnimationManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (animList == null) Logger.Error("anim list was not assigned to animation manager", this);
        else
        {
    ...
    public void PlayAnimation(string name, bool canBePlayedOver = true)
    {
        if(animList==null)
        {
            Logger.Warning($"Anim list was not assigned");
            return;
        }
    ...
    {
        if (name == "Empty" || animList==null) return 0;
        float clipSpeed = 0;
